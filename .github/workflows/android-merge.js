...
async function tagAndPushEA(github, owner, repo, execa) {
    ...
    console.log(`New tag: ${newTag}`);
    console.info('Pushing tags to GitHub ...');
    await execa("git", ["remote", "add", "android", "https://codeberg.org/yuzu-emu/yuzu-android.git"]);
...
async function resetBranch(execa) {
    ...
    let hasFailed = false;
    try {
        await execa("git", ["remote", "add", "source", "https://codeberg.org/yuzu-emu/yuzu.git"]);
...
async function getMainlineTag(execa) {
    ...
    let hasFailed = false;
    try {
        await execa("git", ["remote", "add", "mainline", "https://codeberg.org/yuzu-emu/yuzu-android.git"]);
...
async function mergebot(github, context, execa) {
    ...
    await fetchPullRequests(pulls, "https://codeberg.org/yuzu-emu/yuzu", execa);
